...
- Import GOV.UK macros at the top of each template, one per line, e.g. `{% from "govuk/components/input/macro.njk" import govukInput %}`. Macro paths track GOV.UK Frontend {{GOVUK_FRONTEND_VERSION}}; if a macro path looks wrong, check the installed version rather than guessing. Layout chrome (blue header, service navigation) is documented in `docs/standards/govuk-frontend.md` — run `npm run check:govuk-frontend` periodically.
